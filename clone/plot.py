...
def save_to_csv(obj, filename):
    df = pd.DataFrame(obj)
    df.to_csv('images/data/' + filename + '.csv')
...
def plot_training_loss_stats(train_vector, valid_vector):
    filename = 'plot_training_loss_stats'
...
def plot_training_acc_stats(train_vector, valid_vector):
    filename = 'plot_training_acc_stats'
    save_to_csv({'v1': train_vector, 'v2': valid_vector}, filename)
    epochs = len(train_vector)
...
def plot_confusion_matrix(predicts, trues):
    filename = 'plot_confusion_matrix'
