...
from .combined_model import combine_or
from .metrics import calculate_java_metrics
...
def evaluate(code_pairs, method_name):
    # todo temporary test of model evaluation
    # input_path = root + lang + '/test/source_code.pkl'
    # source = pd.read_pickle(input_path)
    # print(len(source))
    # code_pairs = []
    # for iii, row in source.iterrows():
    #     code_pairs.append([row['source_code_x'], row['source_code_y']])
    # code_pairs = code_pairs[0:32]

    print('evaluate equivalence')
    ...
    print('eval codes')
    results = []
    # ast1, ast2, metrics1, metrics2 = get_constructed_info(source)
    ast1 = []
    ast2 = []
    metrics1 = []
    metrics2 = []
    ...
    metrics_predicted = (ast_output.data > 0.5).cpu().numpy()
    result = combine_or(ast_predicted, metrics_predicted)
...
def construct_code_info(pair, method_name):
    ...
    metrics1 = calculate_java_metrics(ast1)
    metrics2 = calculate_java_metrics(ast2)
...
def parse_program(source_code):
    # tokens = javalang.tokenizer.tokenize(source_code, False)
